...
describe('CU-01 login screen', () => {
  ...
  function component() {
    ...
    component.form.setValue({ usernameOrEmail: ' admin ', password: ' password ', rememberMe: true });
    return component;
  }
  it('passes remember me and trims the identifier without changing the password', () => {
    ...
    login.submit();
    expect(auth.login).toHaveBeenCalledWith({ usernameOrEmail: 'admin', password: ' password ' }, true);
  ...
  it('shows the account lock message returned by the backend', () => {
    auth.login.mockReturnValue(throwError(() => new HttpErrorResponse({ status: 423,

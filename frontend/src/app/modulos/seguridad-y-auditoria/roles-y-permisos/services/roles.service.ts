...
import { Asignacion, Permiso, Rol, UsuarioResumen } from '../models/roles.model';
...
export class RolesService {
  ...
  rolesDeUsuario(usuarioId: number): Observable<Asignacion[]> {
    return this.http.get<Asignacion[]>(`${this.base}/usuarios/${usuarioId}/roles`);
  }

  asignar(usuarioId: number, rolId: string): Observable<void> {
    return this.http.post<void>(`${this.base}/usuarios/${usuarioId}/roles`, { rolId });
  }

  quitar(usuarioId: number, rolId: string): Observable<void> {

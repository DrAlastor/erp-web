...
import { Asignacion, Rol, UsuarioResumen } from '../models/roles.model';
...
export class AsignarRolComponent {
  ...
  elegirUsuario(evento: Event): void {
    ...
    this.error.set(null);
    this.asignados.set([]);
    this.usuarioId.set(valor ? Number(valor) : null);

    if (valor) {
      this.servicio.rolesDeUsuario(Number(valor)).subscribe({

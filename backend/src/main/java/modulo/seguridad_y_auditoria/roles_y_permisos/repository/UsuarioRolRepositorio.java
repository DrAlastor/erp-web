...
public interface UsuarioRolRepositorio extends JpaRepository<UsuarioRol, UsuarioRolId> {
    ...
    /**
     * Roles asignados a varios usuarios de una misma empresa, en una sola consulta.
     *
     * <p>La usa el listado paginado de cuentas de la CU-02, que de otro modo tendría que
     * preguntar por los roles de cada usuario de la página. El resultado viene ordenado para
     * que la respuesta sea estable.
     */
    @Query("""
            select new modulo.seguridad_y_auditoria.roles_y_permisos.dto.RolAsignadoDto(
                        ur.id.usuarioId, r.id, r.nombre)
            from RbacUsuarioRol ur
            join ur.rol r
            where ur.id.usuarioId in :usuarioIds
              and r.empresaId = :empresaId
            order by ur.id.usuarioId, r.nombre
            """)
    List<RolAsignadoDto> findRolesDe(@Param("usuarioIds") Collection<Long> usuarioIds,

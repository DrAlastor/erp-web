...
public class ServicioAutorizacion {
    ...
    private final Map<UUID, Set<String>> cachePorUsuario = new ConcurrentHashMap<>();
    ...
    @Transactional(readOnly = true)
    public boolean tienePermiso(UUID usuarioId, String codigoPermiso) {
    ...
    public void invalidarCache(UUID usuarioId) {
        if (usuarioId != null) {
    ...
    public void invalidarCacheDeRol(UUID rolId) {
        ...
        for (UUID usuarioId : usuarioRolRepositorio.findUsuarioIdsPorRol(rolId)) {
            cachePorUsuario.remove(usuarioId);
        }
    }
    ...
    private Set<String> leerDeLaBase(UUID usuarioId) {
